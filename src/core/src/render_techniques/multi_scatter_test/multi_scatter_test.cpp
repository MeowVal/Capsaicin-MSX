...
bool MultiScatterTests::init(CapsaicinInternal const &capsaicin) noexcept 
{
    ...
    if (lightBuilder->getDirectionalLightCount() == 0)
    {
        auto             scene = capsaicin.getScene();
        sun_   = gfxSceneCreateLight(scene);
        sun_->type             = kGfxLightType_Directional;
        sun_->direction        = normalize(float3(0.7071f, -0.0f, 0.0f));
        sun_->color            = float3(1.0f, 0.95f, 0.85f);
        sun_->intensity        = 100.0f;
        sun_->range            = 0.0f; // unused for directional
    }
...
void MultiScatterTests::render([[maybe_unused]] CapsaicinInternal &capsaicin) noexcept
{
    ...
    gfxProgramSetParameter(gfx_, shadingProgram_, "g_TextureSampler", capsaicin.getLinearWrapSampler());
