...
map<string, Medium> media;
map<string, Interface> interfaces;

__declspec(dllexport) map<string, Medium>& get_media() { return media; }
__declspec(dllexport) map<string, Interface>& get_interfaces() { return interfaces; }
...
void Medium::fill_spectral_data()
{
  if(turbid && ior[spectrum].size() != scattering[spectrum].size())
  {
    if(ior[spectrum].size() < scattering[spectrum].size())
    {
      ...
      else
      {
        ...
        copy(begin(old_ior), end(old_ior), &ior[spectrum][0]);
      }
    }
    else
    {
      ...
      else
      {
        ...
        copy(begin(old_sca), end(old_sca), &scattering[spectrum][0]);
      }
    }
  }
  ...
  asymmetry[spectrum].wavelength = ior[spectrum].wavelength;
  asymmetry[spectrum].step_size = ior[spectrum].step_size;
  albedo[spectrum].wavelength = ior[spectrum].wavelength;
  albedo[spectrum].step_size = ior[spectrum].step_size;
  extinction[spectrum].wavelength = ior[spectrum].wavelength;
  extinction[spectrum].step_size = ior[spectrum].step_size;
  reduced_sca[spectrum].wavelength = ior[spectrum].wavelength;
  reduced_sca[spectrum].step_size = ior[spectrum].step_size;
  reduced_ext[spectrum].wavelength = ior[spectrum].wavelength;
  reduced_ext[spectrum].step_size = ior[spectrum].step_size;
  reduced_alb[spectrum].wavelength = ior[spectrum].wavelength;
  reduced_alb[spectrum].step_size = ior[spectrum].step_size;
  ...
  if(turbid)
  {
    if(asymmetry[spectrum].size() != samples)
    {
      ...
      else
      {
        ...
        if(samples > old_asym.size())
          copy(begin(old_asym), end(old_asym), &asymmetry[spectrum][0]);
        else
          copy(begin(old_asym), begin(old_asym) + samples, &asymmetry[spectrum][0]);
      }
    }
  }
  else
  {
    if(scattering[spectrum].size() != samples)
      scattering[spectrum].resize(samples, 0.0);
    if(asymmetry[spectrum].size() != samples)
      asymmetry[spectrum].resize(samples, 0.0);
  }
  ...
  {
    albedo[spectrum].resize(samples);
  ...
  {
    reduced_alb[spectrum].resize(samples);
