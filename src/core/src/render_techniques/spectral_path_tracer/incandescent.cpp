...
#include <third_party/KTX-Software/external/astc-encoder/Source/astcenc_mathlib.h>
...
Medium blackbody(double T)         // temperature [K]
{
  ...
  for(unsigned int i = 0; i < emission.size(); ++i)
  {
    ...
    emission[i] = 2.0* astc::PI*c*c*h/(pow(lambda, 5)*(exp(h*c/(lambda*k*T)) - 1.0))*25.0e-9;
  }
                    ...
                    double filament_area) // filament surface area [m^2]
{
  ...
  //cout << "incandescent (T = " << T << ") emission: [";
  for(unsigned int i = 0; i < emission.size(); ++i)
  {
    emission[i] *= filament_area/bulb_area;
  //  cout << emission[i] << (i < emission.size() - 1 ? ", " : "");
  }
  //cout << "]" << endl;
